...
export const WebviewRequestSchema = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('cancelQuery') }),
  ...
  z.object({ type: z.literal('updateTablePresentation'), tableId: IdSchema, presentation: TablePresentationSchema }),
  z.object({ type: z.literal('updateTheme'), theme: ProjectThemeSchema }),
  ...
  z.object({ type: z.literal('renameReport'), reportId: IdSchema, name: z.string().min(1).max(200), description: z.string().max(2000) }),
  z.object({ type: z.literal('renamePage'), reportId: IdSchema, pageId: IdSchema, name: z.string().min(1).max(200), description: z.string().max(2000) }),
  ...
  z.object({ type: z.literal('duplicateVisual'), reportId: IdSchema, pageId: IdSchema, visualId: IdSchema }),
  z.object({ type: z.literal('reorderVisual'), reportId: IdSchema, pageId: IdSchema, visualId: IdSchema, toIndex: z.number().int().nonnegative() }),
